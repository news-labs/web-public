...
const wranglerFile = join(root, "workers/admin-web-api/wrangler.stg.jsonc");
...
const D1_NAME = "nl-web-db-admin-stg";

if (!TOKEN || !ACCOUNT_ID) { console.error("Missing CLOUDFLARE_API_TOKEN or CLOUDFLARE_ACCOUNT_ID"); process.exit(1); }

function run(cmd, opts = {}) {
  console.log(`→ ${cmd}`);
  return execSync(cmd, { cwd: opts.cwd ?? wranglerCwd, encoding: "utf8", stdio: "inherit" });
}

async function cfJson(path) {
  const res = await fetch(`https://api.cloudflare.com/client/v4${path}`, { headers: { Authorization: `Bearer ${TOKEN}`, "Content-Type": "application/json" } });
...
async function getOrCreateD1() {
  ...
  if (existing) { console.log(`✅ D1 ${D1_NAME} (${existing.uuid})`); return existing.uuid; }
...
function patchWranglerStg(d1Id) {
  let content = readFileSync(wranglerFile, "utf8");
  content = content.replaceAll("PLACEHOLDER_STG", d1Id);
  writeFileSync(wranglerFile, content);
  console.log(`📝 patched ${wranglerFile}`);
}

async function main() {
  console.log("\n🚀 Web Admin staging provisioning\n");
  run("node scripts/preflight-cloudflare.mjs", { cwd: root });
  const d1Id = await getOrCreateD1();
  patchWranglerStg(d1Id);
  if (applyMigrations) run(`pnpm exec wrangler d1 migrations apply ${D1_NAME} --remote -c wrangler.stg.jsonc`);
  writeFileSync(join(root, "scripts/stg-resource-ids.web.json"), JSON.stringify({ d1Id, updatedAt: new Date().toISOString() }, null, 2));
  console.log("\n✅ Provision complete.\n");
}
